...
@Composable internal fun CabinetForm(kind:String, vm:CabinetViewModel, close:()->Unit) {
    ...
    val choices=when(kind) {
        ...
        "owner-lead"->mapOf("status" to listOf("new" to "Получена","reviewing" to "Рассматриваем","need_info" to "Нужна информация","qualified" to "Квалифицирована","accepted" to "Принята","rejected" to "Не реализуем","converted" to "Проект создан","archived" to "Архив"))
