...
class CabinetUiE2ETest {
    ...
    @Test fun registrationProjectMessagesOwnerAndRecreation() {
        ...
        waitFor("auth-password-reset/confirm");tap("auth-password-reset/confirm")
        fill("cabinet-token",runBlocking(Dispatchers.IO){mailCode(email,"password_reset")})
        fill("cabinet-password",TEST_PASSWORD);fill("cabinet-password-repeat",TEST_PASSWORD);hideKeyboard();shot("registration-reset")
        tap("cabinet-auth-submit")
