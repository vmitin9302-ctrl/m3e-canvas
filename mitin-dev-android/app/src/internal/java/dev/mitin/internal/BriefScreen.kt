...
@Composable fun BriefScreen(vm: BriefViewModel) {
    ...
    when {
        state == null -> {
            ...
            briefServices.forEach { (key,label) -> FilterChip(selected=service==key,onClick={service=key},label={Text(label)},enabled=!vm.busy,modifier=Modifier.testTag("brief-service-$key")) }
            Heading("Ориентир бюджета")
            briefBudgets.forEach { (key,label) -> FilterChip(selected=budget==key,onClick={budget=key},label={Text(label)},enabled=!vm.busy,modifier=Modifier.testTag("brief-budget-$key")) }
        ...
        state.submitted -> {
            ...
            if(vm.contactStep && state.finalBrief != null) {
                ...
                state.messages.forEach { message -> InfoCard(if(message.role=="user") "Вы" else "AI",message.text) }
